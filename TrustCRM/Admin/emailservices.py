...
#Send mail to  client from complaints
    def complaint_client(self,complaint_id,name,remail,description):
        try:
            ...
            print("EXCEPTION-----------------------") 
        return message  
#Reassign to manager send email
...
#Email rep about nonfunded ticket
    def mail_after_one_week(self,ticket):
        repid=""
        repname=""
        repemail=""
        salesrep=TblSaleslead.objects.filter(ticket_no=ticket)
        print("Sales rep object====",salesrep)
        for rep in salesrep.iterator():
            print("Rep ======",rep.salesrepid)
            repid=rep.salesrepid
        if repid!="":
            repdetails=TblUser.objects.filter(userid=repid)
            for details in repdetails.iterator():
                repname=details.username
                repemail=details.email
        print("rep name====",repname)
        print("rep email====",repemail)
